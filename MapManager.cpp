...
MapManager::MapManager() :
	coin_animation(CELL_DIMENSION, "Images/Coin.png", COIN_ANIMATION_SPEED),
	question_block_animation(CELL_DIMENSION, "Images/QuestionBlock.png", QUESTION_BLOCK_ANIMATION_SPEED)
{
...
void MapManager::add_brick_particles(const unsigned short i_x, const unsigned short i_y)
{
	...
	brick_particles.push_back(Object(i_x, i_y, -0.25f * BRICK_PARTICLE_VELOCITY, -1.5f * BRICK_PARTICLE_VELOCITY));
	brick_particles.push_back(Object(i_x + 0.5f * CELL_DIMENSION, i_y, 0.25f * BRICK_PARTICLE_VELOCITY, -1.5f * BRICK_PARTICLE_VELOCITY));
	brick_particles.push_back(Object(i_x, i_y + 0.5f * CELL_DIMENSION, -0.5f * BRICK_PARTICLE_VELOCITY, -BRICK_PARTICLE_VELOCITY));
	brick_particles.push_back(Object(i_x + 0.5f * CELL_DIMENSION, i_y + 0.5f * CELL_DIMENSION, 0.5f * BRICK_PARTICLE_VELOCITY, -BRICK_PARTICLE_VELOCITY));
}

void MapManager::add_question_block_coin(const unsigned short i_x, const unsigned short i_y)
{
	question_block_coins.push_back(Object(i_x, i_y, 0, COIN_JUMP_VELOCITY));
}
...
{
	unsigned short map_end = ceil((SCREEN_WIDTH + i_view_x) / static_cast<float>(CELL_DIMENSION));
	unsigned short map_height = floor(map_sketch.getSize().y / 3.f);
	unsigned short map_start = floor(i_view_x / static_cast<float>(CELL_DIMENSION));
	...
	if (0 == i_draw_background)
	{
		...
		{
			coin_animation.for_place(question_block_coin.x, question_block_coin.y);
	...
	for (unsigned short a = map_start; a < map_end; a++)
	{
		for (unsigned short b = 0; b < map_height; b++)
		{
			...
			else if (Cell::Empty != map[a][b])
			{
				...
				{
					coin_animation.for_place(CELL_DIMENSION * a, CELL_DIMENSION * b);
					coin_animation.draw(i_window);
				}
				else if (Cell::QuestionBlock == map[a][b])
				{
					question_block_animation.for_place(CELL_DIMENSION * a, CELL_DIMENSION * b);
	...
	if (0 == i_draw_background)
	{
		for (const Object& brick_particle : brick_particles)
		{
			...
			cell_sprite.setTextureRect(sf::IntRect(0.25f * CELL_DIMENSION, CELL_DIMENSION * (0.25f + 2 * i_underground), 0.5f * CELL_DIMENSION, 0.5f * CELL_DIMENSION));
			i_window.draw(cell_sprite);
		}
	}
}
...
{
	map_sketch.loadFromFile("Images/LevelSketch" + std::to_string(static_cast<unsigned short>(i_current_level)) + ".png");
}
...
void MapManager::update()
{
	...
	{
		question_block_coin.up_motion += GRAVITY_ACCELERATION;

		question_block_coin.y += question_block_coin.up_motion;
	}

	for (Object& brick_particle : brick_particles)
	{
		brick_particle.up_motion += GRAVITY_ACCELERATION;

		brick_particle.x += brick_particle.side_motion;
		brick_particle.y += brick_particle.up_motion;
	}
	...
	{
		return 0 <= i_question_block_coin.up_motion;
...
std::vector<unsigned char> MapManager::map_collision(const std::vector<Cell>& i_check_cells, const sf::FloatRect& i_hitbox) const
{
	...
	for (short a = floor(i_hitbox.top / CELL_DIMENSION); a <= floor((ceil(i_hitbox.height + i_hitbox.top) - 1) / CELL_DIMENSION); a++)
	{
		output.push_back(0);

		for (short b = floor(i_hitbox.left / CELL_DIMENSION); b <= floor((ceil(i_hitbox.left + i_hitbox.width) - 1) / CELL_DIMENSION); b++)
		{
			if (0 <= b && b < map.size())
			{
				if (0 <= a && a < map[0].size())
				{
					if (i_check_cells.end() != std::find(i_check_cells.begin(), i_check_cells.end(), map[b][a]))
					{
						...
						output[a - floor(i_hitbox.top / CELL_DIMENSION)] += pow(2, floor((ceil(i_hitbox.left + i_hitbox.width) - 1) / CELL_DIMENSION) - b);
					}
				}
			}
